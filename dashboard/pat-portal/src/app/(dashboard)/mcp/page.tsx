...
import { type MCPServer, type MCPServerCreate, mcpApi } from "@/lib/api/mcp";
...
function RegisterServerModal({ onClose }: { onClose: () => void }) {
  ...
  const set = (k: keyof MCPServerCreate, v: unknown) =>
    setForm((f) => ({ ...f, [k]: v }));
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="number"
                value={form.startup_timeout_sec ?? 30}
                onChange={(e) =>
                  set(
                    "startup_timeout_sec",
          ...
          <div className="flex items-center gap-3">
            <input
              type="checkbox"
              id="oauth"
              checked={!!form.supports_oauth}
              onChange={(e) => set("supports_oauth", e.target.checked)}
              className="rounded border-zinc-600 bg-zinc-800"
            />
            <label htmlFor="oauth" className="text-sm text-zinc-300">
              Supports OAuth
            </label>
          </div>
          ...
          {form.supports_oauth && (
            <div className="space-y-3 rounded-lg bg-zinc-800/50 border border-zinc-700/50 p-4">
              <p className="text-xs text-zinc-500">
                Required for providers like Google. Leave empty for Notion /
                Eraser.
              </p>
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("oauth_client_id", e.target.value || undefined)
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("oauth_client_secret", e.target.value || undefined)
                  }
                  placeholder="GOCSPX-…"
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-zinc-500 placeholder-zinc-600"
                />
        ...
        <td className="px-6 py-4">
          <div className="flex items-center gap-2">
            {!isConnected ? (
              ...
              >
                {connectMutation.isPending || oauthMutation.isPending ? (
            ...
            {server.supports_oauth && (
              <button
                ...
                title={
                  isExpired ? "Re-authorize (token expired)" : "Authorize OAuth"
                }
              >
                {oauthMutation.isPending || reconnectMutation.isPending ? (
                  <Loader2 size={12} className="animate-spin" />
            ...
            <div className="pt-3">
              ...
              {tools && tools.length === 0 && (
                <p className="text-xs text-zinc-600">
                  No tools synced yet. Click the <strong>Sync Tools</strong>{" "}
                  button above to discover and cache tools from this server.
                </p>
              )}
              ...
              {tools && tools.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                  {tools.map((tool) => (
                    ...
                    >
                      <div className="text-xs font-medium text-zinc-200 font-mono">
                        {tool.tool_name}
                      </div>
                      {tool.description && (
...
export default function MCPPage() {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      {showRegister && (
        <RegisterServerModal onClose={() => setShowRegister(false)} />
      )}
      ...
      <div className="flex flex-wrap gap-4 text-xs text-zinc-500">
        <span className="flex items-center gap-1.5">
          ...
          connected — active, agent will use this server
        </span>
        <span className="flex items-center gap-1.5">
