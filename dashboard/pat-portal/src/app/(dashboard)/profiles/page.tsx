...
import { profilesApi, ProfileCreate } from '@/lib/api/profiles';
import { promptsApi } from '@/lib/api/prompts';
import { Plus, Bot, Settings2, X, Loader2 } from 'lucide-react';
...
function CreateProfileModal({ onClose }: { onClose: () => void }) {
  ...
  const { data: prompts } = useQuery({ queryKey: ['prompts'], queryFn: promptsApi.getPrompts });

  const mutation = useMutation({
    mutationFn: () => profilesApi.createProfile({ ...form, description: form.description || null }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['profiles'] }); onClose(); },
  });

  const set = (k: keyof ProfileCreate, v: any) => setForm(f => ({ ...f, [k]: v }));

  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-lg p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-base font-semibold text-white">Create Agent Profile</h2>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-200"><X size={18} /></button>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1.5">Name *</label>
            <input type="text" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Research Agent"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.model_name} onChange={e => set('model_name', e.target.value)}
                ...
                {['gpt-4.1-mini', 'gpt-4.1', 'gpt-4o', 'gpt-4o-mini'].map(m => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">Max Turns</label>
              <input type="number" min="1" value={form.max_turns} onChange={e => set('max_turns', parseInt(e.target.value))}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-zinc-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">System Prompt</label>
              <select value={form.prompt_id ?? ''} onChange={e => set('prompt_id', e.target.value || null)}
                className="w-full bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-zinc-500">
        ...
        {mutation.isError && (
          <p className="mt-4 text-xs text-red-400">{(mutation.error as any)?.response?.data?.detail || 'Something went wrong.'}</p>
        )}
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400 mt-1">
            {isLoading ? 'Loading...' : `${profiles?.length ?? 0} profiles configured.`}
          </p>
        </div>
      ...
      {isLoading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => <div key={i} className="h-44 rounded-xl border border-zinc-800 bg-zinc-900/30 animate-pulse" />)}
        </div>
      )}
      {isError && <p className="text-sm text-red-400">Failed to load profiles.</p>}
      {!isLoading && !isError && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {profiles?.map((profile) => (
            <Link key={profile.id} href={`/profiles/${profile.id}`} className="block group">
              <div className="p-6 rounded-xl border border-zinc-800 bg-[#0A0A0A] hover:border-zinc-600 transition-colors h-full flex flex-col">
                ...
                <p className="text-sm text-zinc-500 mt-1 line-clamp-2 flex-1">{profile.description || 'No description.'}</p>
                <div className="flex items-center gap-4 mt-6 pt-4 border-t border-zinc-800/50">
                  <div className="flex items-center gap-1.5 text-xs font-medium text-zinc-400">
                    <span className="w-2 h-2 rounded-full bg-blue-500" />{profile.model_name}
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                    <Settings2 size={14} />v{profile.version}
                  </div>
                  <div className="ml-auto text-xs text-zinc-600">{profile.temperature} temp</div>
                </div>
              </div>
            </Link>
          ))}
