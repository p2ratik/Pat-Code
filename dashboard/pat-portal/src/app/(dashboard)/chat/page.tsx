...
import { Bot, MessageSquare, Plus, Send, User as UserIcon } from "lucide-react";
import { useEffect, useRef, useState } from "react";
import { authApi } from "@/lib/api/auth";
import { chatApi } from "@/lib/api/chat";
import { ConversationMessage, conversationsApi } from "@/lib/api/conversations";
...
export default function ChatPage() {
  ...
  const { data: profileTools } = useQuery({
    ...
    queryFn: () =>
      userProfile
        ? import("@/lib/api/tools").then((m) =>
            m.toolsApi.getProfileTools(userProfile.id),
          )
        : null,
    enabled: !!userProfile,
  });
              ...
              <div className="text-center">
                ...
                {userProfile && (
                  <p className="text-xs mt-1 text-zinc-600">
                    Using{" "}
                    <span className="text-zinc-500">{userProfile.name}</span>{" "}
                    profile ·{" "}
        ...
        <div className="p-4 border-t border-zinc-800 bg-[#0A0A0A]">
          <div className="max-w-3xl mx-auto relative">
            ...
            <button
              onClick={handleSend}
              disabled={
                chatMutation.isPending || !input.trim() || loadingHistory
              }
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="text-sm font-medium text-zinc-200">
              {userProfile?.name ?? (
                <span className="text-zinc-600 italic">No profile</span>
              )}
            </div>
          </div>
